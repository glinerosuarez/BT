...
class AshbyAdapter:
            ...
            () => {
              ...
              for (const entry of fieldEntries) {
                ...
                for (const el of Array.from(entry.querySelectorAll('input, textarea, select'))) {
                  ...
                  if (inputType === 'file' && !el.getAttribute('id')) continue; // Skip Ashby's optional autofill uploader.
    ...
    def _wait_for_submission_state(self, page) -> None:
        try:
            ...
            # A timeout means the adapter will preserve an ambiguous state rather than assume success.
            pass
